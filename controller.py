...
def get_fl_rss_feed(n=100):
    '''
    returns last n movies from filelist RSS feed.
    n is default 100
    '''
...
def durataPy(oraStart):
    ...
    durata = datetime.datetime.strptime(oraEnd.time().strftime('%H:%M:%S'), '%H:%M:%S') - datetime.datetime.strptime(
        oraStart.time().strftime('%H:%M:%S'), '%H:%M:%S')

    return durata


log_file_path = '/home/matei/Documents/Python/MoviePediaV2/logs/{0}-{1}_{2}.log'.format(datetime.date.today().year,
...
print
oraStart.strftime('%Y-%m-%d %H:%M:%S'), '- Incepe verificarea de torrenti noi \n'
...
for rss_item in rss_data.entries:
    ...
    if filelist.rss_date not in list_rss:
        ...
        if filelist.rss_id_imdb is not None:
            ...
            movie_data = {'seen_type': '', 'title': '', 'year': '', 'genre': '', 'rated': '', 'country': '',
                          'runtime': '', 'imdb_score': '', 'score': '', 'rott_score': '', 'meta_score': '', 'ovrw': '',
                          'director': '', 'cast': '', 'poster': '', 'trailer': '', 'my_imdb_score': '', 'seen_date': ''}
        else:
            movie_data = {'seen_type': '0', 'title': filelist.rss_title, 'year': filelist.rss_year, 'genre': '',
                          'rated': '', 'country': '', 'runtime': '', 'imdb_score': '', 'score': '', 'rott_score': '',
                          'meta_score': '', 'ovrw': '', 'director': '', 'cast': '',
                          'poster': 'https://www.bioskoptoday.com/wp-content/themes/bioskoptodaytheme/images/no-poster.png',
                          'trailer': '', 'my_imdb_score': '', 'seen_date': ''}

        wsheet_rss.append_row(
            [filelist.id_filelist, filelist.rss_date, filelist.rss_id_imdb, filelist.rss_name, filelist.rss_link,
             filelist.rss_resolution, filelist.rss_quality, filelist.rss_year, filelist.rss_freeleech,
             filelist.rss_internal, filelist.rss_size, oraStart])

        trnt_data = {'resolution': filelist.rss_resolution, 'size': int(filelist.rss_size),
                     'freeleech': filelist.rss_freeleech, 'trend': ''}
        ...
        if mtls.find is False:
            mtls.add_movie_to_xml(cfg.xml_trnt_path, filelist.rss_id_imdb, filelist.id_filelist, movie_data=movie_data,
                                  trnt_data=trnt_data)
        elif mtls.find is True:
            mtls.find_trnt_elem_xml(cfg.xml_trnt_path, 'trnt', 'id', filelist.id_filelist)
            if mtls.find is False:
                mtls.new_trnt_link_xml(cfg.xml_trnt_path, filelist.rss_id_imdb, filelist.id_filelist,
                                       trnt_data=trnt_data)
        mtls.xml_pritify(cfg.xml_trnt_path)

print
'{0} feed-uri noi'.format(len(list_rss_new))
...
if list_id_imdb_new:
    for new_id_imdb in list_id_imdb_new:
        ...
        if new_id_imdb not in list_tmdb:
            ...
            wsheet_tmdb.append_row(
                [tmdb.id_imdb, tmdb.type_tmdb, tmdb.id_tmdb, tmdb.title, tmdb.year, tmdb.lang, tmdb.genre, tmdb.score,
                 tmdb.poster, tmdb.ovrw, tmdb.runtime, tmdb.country, tmdb.url, oraStart])

            attrs = vars(tmdb)
            for item in attrs.items():
                mtls.update_trnt_child_xml(cfg.xml_trnt_path, tmdb.id_imdb, '{0}'.format(item[0]),
                                           '{0}'.format(item[1]))
            mtls.xml_pritify(cfg.xml_trnt_path)
...
if list_id_imdb_new:
    for new_id_imdb in list_id_imdb_new:
        ...
        if new_id_imdb not in list_omdb:
            ...
            wsheet_omdb.append_row(
                [omdb.id_imdb, omdb.title, omdb.year, omdb.genre, omdb.rated, omdb.country, omdb.lang, omdb.awards,
                 omdb.imdb_score, omdb.rott_score, omdb.meta_score, oraStart])
...
list_new, list_trnt, list_seen = mtls.read_filelist_xml(cfg.xml_trnt_path, cfg.movie_template_path,
                                                        cfg.trnt_template_path)
...
if email_body:
    print
    'trimit mail'
    mtls.send_email('TeleCinemateca', sys_email.username, cfg.email_list_debug, mail_subject, email_body, '',
                    sys_email.hostname, sys_email.username, sys_email.password)
else:
    print
    'nimic de trimis pe mail'