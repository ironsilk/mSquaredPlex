...
class Mtls:
	...
	*******************************************************************************'''
	def generate_html_list (self, vColor, vCount, vTitle, vYear, vIMDBid, vPoster, vResolution, vGenre, vRated, vCountry, vRuntime, vIMDBScore, vTMDBScore, vRottenTScore, vMetaCScore, vPlot, vTrailer, vDirector, vActors, vSize, vFreeL, vFLISid, vMyScore, vMyScoreDate):
		...
		if vFLISid is not None:
			sDW = (u'<tr><td style="text-align: center;" colspan="2"><strong><a id="btnDw" href="http://gsmatei.go.ro:5080/dwtrnt?idFlist={0}&idFolder=new">DOWNLOAD</a>&emsp;|&emsp;<a id="btnSd" href="http://gsmatei.go.ro:5080/dwtrnt?idFlist={0}&idFolder=seed">SEED ONLY</a></strong></td></tr>'.format(vFLISid))
		else:
			sDW = ''
			...
			if sGenre is not None:
				...
				for genre in genreDict:
					if genre.lower() in sGenre.lower():
						sGenre = sGenre.replace(genre, '<span style="color: #ff0000;"><strong>{0}</strong></span>'.format(genre))
			...
			if sCountry is not None:
				...
				for country in countryDict:
					if country.lower() in sCountry.lower():
						sCountry = sCountry.replace(country, '<span style="color: #ff0000;"><strong>{0}</strong></span>'.format(country))
		except Exception as vErr:
		...
		if vMyScore is not None and vMyScoreDate is not None:
			vMyScoreHTML = '&ensp;{0}:{1}<strong>&ensp;{2}&ensp;</strong></span><span style="color: #ccc;">|</span>'.format(vMyScoreDate, styleMScore, self.n2d(vMyScore))
		else:
			vMyScoreHTML = ''
		...
		try:
			...
			html = html + (u'\n<tr><td style="font-size: 11px; color: #666;">{0}&ensp;<span style="color: #ccc;">|</span>&ensp;{1}&ensp;<span style="color: #ccc;">|</span>&ensp;{2}&ensp;<span style="color: #ccc;">|</span>&ensp;{3}&ensp;<span style="color: #ccc;">|</span>&ensp;{4}{5}&ensp;<span style="color: #ccc;">|</span></td></tr>\n'
				.format(self.n2d(vResolution), self.n2d(sGenre), self.n2d(vRated), self.n2d(sCountry), self.n2d(vRuntime), sSize))
			html = html + (u'\n<tr><td style="font-size: 11px; color: #666;">IMDB:{0}<strong>&ensp;{1}&ensp;</strong></span><span style="color: #ccc;">|</span>&ensp;TMDB:{2}<strong>&ensp;{3}&ensp;</strong></span><span style="color: #ccc;">|</span>&ensp;Rotten T:{4}<strong>&ensp;{5}&ensp;</strong></span><span style="color: #ccc;">|</span>&ensp;Meta C:{6}<strong>&ensp;{7}&ensp;</strong></span><span style="color: #ccc;">|</span>{8}</td></tr>\n'
				.format(SStyleIMDB, self.n2d(vIMDBScore), SStyleTMDB, self.n2d(vTMDBScore), SStyleRottenT, self.n2d(vRottenTScore), SStyleMetaC, self.n2d(vMetaCScore), vMyScoreHTML))
	...
	def xml_pritify(self, xml_file_path):
		...
		xml_raw = ''.join([s for s in ET.tostring(root).splitlines(True) if s.strip()])	# remove empty lines from string
	...
	def count_xml(self, xml_file_path):
		...
		print 'total filme:', len(root.getchildren())
		print 'din care'
		...
		self.new_movies = len(root.getchildren())
			...
			if key in score_list:
				...
				for genre in genre_bad_list:
					if genre.lower() in value.lower():
						value =value.replace(genre, '<span style="color: #ff0000;"><strong>{0}</strong></span>'.format(genre))

			elif key == 'country':
				for country in country_bad_list:
					if country.lower() in value.lower():
						value = value.replace(country, '<span style="color: #ff0000;"><strong>{0}</strong></span>'.format(country))
			...
			if isinstance(value, dict):
				for key2, value2 in value.items():
					...
					if value2 == 'True':
						...
						trnt_tbody = trnt_tbody.replace('trnt_{0}'.format(key2), '<img src="https://filelist.io/styles/images/tags/freeleech.png" alt="freeleech" />')
	...
	def generate_email_html (self, template_path, list_new, list_trnt, list_seen, durata):
		...
		if 'email_filelist.html' in template_path:
			f = open('/home/matei/Documents/Python/MoviePediaV2/logs/eml_filelist.html', 'w+')
		elif 'email_new_movies.html' in template_path:
			f = open('/home/matei/Documents/Python/MoviePediaV2/logs/eml_new_movies.html', 'w+')
		else:
			f = open('/home/matei/Documents/Python/MoviePediaV2/logs/eml_temp.html', 'w+')
		f.write(email_body)
		f.close
		...
		for movie in root:
			...
			if movie.find("title").text is None:
				...
				for item in attrs.items():
					self.update_trnt_child_xml(xml_file_path, tmdb.id_imdb, '{0}'.format(item[0]), '{0}'.format(item[1]))
	...
	def read_filelist_xml(self, xml_file_path, movie_template_path, trnt_template_path):
		...
		for movie in root:
			...
			if movie.find("seen_type").text == '0':
				crt_new += 1
				all_movie = {'crt':'{0}'.format(crt_new), 'id_imdb':'{0}'.format(movie.attrib['id_imdb']), 'bck_color':'F4CCCC'}
				all_trnt = {}
				...
				crt_tr += 1
				all_movie = {'crt':'{0}'.format(crt_tr), 'id_imdb':'{0}'.format(movie.attrib['id_imdb']), 'bck_color':'FFF2CC'}
				all_trnt = {}
				for trnt in movie.iter('trnt'):
					#print trnt.attrib['id']
					for subelem in movie.getchildren():
				...
				crt_seen += 1 #CFE2F3
				all_movie = {'crt':'{0}'.format(crt_seen), 'id_imdb':'{0}'.format(movie.attrib['id_imdb']), 'bck_color':'CFE2F3'}
				all_trnt = {}
				for trnt in movie.iter('trnt'):
					#print trnt.attrib['id']
					for subelem in movie.getchildren():
