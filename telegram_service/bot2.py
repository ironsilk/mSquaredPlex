...
CHOOSE_TASK, REGISTER_USER, DOWNLOAD_MOVIE, CHECK_PROGRESS, UPLOAD_ACTIVITY, DOWNLOAD_ACTIVITY, RATE_TITLE = range(7)


# State definitions  DOWNLOAD_MOVIE
CHOOSE_MULTIPLE, CHOOSE_ONE, CONFIRM_REDOWNLOAD_ACTION, SEARCH_FOR_TORRENTS, \
DOWNLOAD_TORRENT, WATCHLIST_NO_TORR = range(7, 13)

# State definitions for CHECK_PROGRESS
DOWNLOAD_PROGRESS = 13

# State definitions for UPLOAD_ACTIVITY
NETFLIX_CSV, RATE_OR_NOT_TO_RATE = 14, 15
...
CHECK_RIDDLE_RESPONSE, CHECK_EMAIL, GIVE_IMDB, CHECK_IMDB = range(18, 22)

# State definitions for aux functions
RESET_CONVERSATION = 23
...
def choose_task(update: Update, context: CallbackContext) -> int:
    ...
    return break_all(update, context)
...
@auth_wrap
def break_all(update: Update, context: CallbackContext) -> None:
    """Dude u messed up :) """
    context.user_data.clear()
    update.message.reply_text(
        f"Ok, so, what do you need?",
...
def main() -> None:
    ...
    download_movie_conversation_handler = ConversationHandler(
        ...
        map_to_parent={
            ConversationHandler.END: CHOOSE_TASK,
            DOWNLOAD_MOVIE: DOWNLOAD_MOVIE,
        }
    )

    check_progress_conversation_handler = ConversationHandler(
        entry_points=[MessageHandler(Filters.text, choose_task)],
        states={
            DOWNLOAD_PROGRESS: [
                MessageHandler(
    ...
    download_activity_conversation_handler = ConversationHandler(
        entry_points=[MessageHandler(Filters.text, choose_task)],
        states={
            DOWNLOAD_CSV: [
                MessageHandler(
                    Filters.text, download_csv
                ),
            ],
        },
        fallbacks=[],
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            DOWNLOAD_ACTIVITY: [download_activity_conversation_handler],
